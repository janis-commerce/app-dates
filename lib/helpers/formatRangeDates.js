 ...
 * @param format - The `format` parameter in the `formatDates` function is used to specify the format
 ...
 * provided
 */
const formatRangeDates = (dates = [], format, options) => {
	...
	return [firstDate, secondDate]
		.map((date) => formatDate(date, format || formatFromLocale, options))
		.join(' - ');
};
