...
class DateHandler {
	...
	format(dates, format) {
		try {
			...
			if (isToday(dateToFormat) || isTomorrow(dateToFormat))
				return formateRelativeDate(dateToFormat, { locale: this.locale });

			return formatDate(dateToFormat, format, { locale: this.locale });
		} catch (reason) {
