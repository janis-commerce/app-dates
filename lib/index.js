...
import locales, { mapperLanguages } from './locales.js';
...
class DateHandler {
	 ...
	 * @param {boolean} [config.debug=false] - config.debug
	 * @example
	...
	constructor(config) {
		this.locale =
			locales[mapperLanguages[String(config?.locale)] || String(config?.locale)] || locales.en;
	 ...
	 */
	setLanguage(lang = '') {
		this.locale = locales[mapperLanguages[String(lang)] || String(lang)] || locales.en;
		setDefaultOptions({ locale: this.locale });
	}
	/**
	 *
	...
	formatDelivery(dates) {
		try {
			...
			if (this.debug) logError(reason.message);
			return null;
		}
	}
